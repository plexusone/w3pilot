...
This SDK provides a native Go interface to the Vibium browser automation platform, which uses the WebDriver BiDi protocol for bidirectional communication with the browser.
...
```bash
go get github.com/agentplexus/vibium-go
```

## Prerequisites

The Vibium clicker binary is required. Set `VIBIUM_CLICKER_PATH` to point to the binary.
