...
Initial release of the WebPilot Client SDK.

## Highlights

**Go client for WebPilot browser automation platform with full feature parity with JavaScript and Python clients.**

This SDK provides a native Go interface to the WebPilot browser automation platform, which uses the WebDriver BiDi protocol for bidirectional communication with the browser.
...
- Set `WEBPILOT_DEBUG=1` to enable structured JSON logging via Go's `slog` package

## Installation

```bash
go get github.com/agentplexus/webpilot
```

## Prerequisites

The WebPilot clicker binary is required. Set `WEBPILOT_CLICKER_PATH` to point to the binary.
...
import (
    ...
    "github.com/agentplexus/webpilot"
)
...
- [README](https://github.com/agentplexus/webpilot/blob/main/README.md) - Full documentation
- [GoDoc](https://pkg.go.dev/github.com/agentplexus/webpilot) - API reference
- [CHANGELOG](https://github.com/agentplexus/webpilot/blob/main/CHANGELOG.md) - Version history
