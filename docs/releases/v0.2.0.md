...
**Repository moved from `grokify/webpilot` to `agentplexus/webpilot`.**
...
```go
// Before
import "github.com/grokify/webpilot"

// After
import "github.com/agentplexus/webpilot"
```

Update your `go.mod`:

```bash
go get github.com/agentplexus/webpilot@v0.2.0
```
...
- Updated all documentation to reference new `agentplexus/webpilot` repository
...
The WebPilot clicker binary is required but not yet publicly distributed. See [Prerequisites](../getting-started/prerequisites.md) for details.
...
1. Update import paths:
   ...
   github.com/grokify/webpilot → github.com/agentplexus/webpilot
   ```

2. Update go.mod:
   ```bash
   go mod edit -replace github.com/grokify/webpilot=github.com/agentplexus/webpilot@v0.2.0
   go mod tidy
   ```
